...
export default function InviteResetPassword() {
  ...
  const validateToken = async () => {
    ...
    try {
      const response = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:3001/api'}/auth/invite/${token}`);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(
        `${import.meta.env.VITE_API_URL || 'http://localhost:3001/api'}/auth/invite/${token}/reset-password`,
        {
          method: 'POST',
          headers: {
