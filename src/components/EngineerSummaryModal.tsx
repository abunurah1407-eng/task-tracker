...
interface EngineerSummaryModalProps {
  isOpen: boolean;
  onClose: () => void;
  yearTasks: Task[];
...
export default function EngineerSummaryModal({
  isOpen,
  onClose,
  yearTasks,
  monthsInSelectedYear,
  selectedYear,
}: EngineerSummaryModalProps) {
  ...
  const exportToPDF = () => {
    ...
    const engineersData = getEngineersSummaryPerEngineer();
    ...
    const reportContent = `
      ...
      <html>
        ...
        <body>
          <div class="header">
            <h1>Engineers Summary Report</h1>
            <p><strong>Year:</strong> ${selectedYear}</p>
            <p><strong>Generated:</strong> ${new Date().toLocaleString()}</p>
          </div>
          
          ${engineersData.map(({ engineer, total, pending, inProgress, completed, months }) => `
            <div class="engineer-card">
              ...
              ` : '<div style="color: #9ca3af; font-style: italic; margin-top: 10px;">No tasks found for this engineer</div>'}
            </div>
          `).join('')}
        </body>
      </html>
  ...
  // Calculate engineers summary per engineer (engineers -> months hierarchy)
  const getEngineersSummaryPerEngineer = () => {
    // Get all unique engineers from yearTasks
    const uniqueEngineers = Array.from(new Set(yearTasks.map(t => t.engineer).filter(Boolean)));
    
    return uniqueEngineers.map(engineer => {
      const engineerTasks = yearTasks.filter(t => t.engineer === engineer);
      
      // Calculate month breakdown for this engineer - include all months
      const monthCounts = monthsInSelectedYear.map(month => {
        const monthTasks = engineerTasks.filter(t => t.month === month);
        return {
          month,
      ...
      // Calculate totals for the engineer
      const engineerTotal = engineerTasks.length;
      const engineerPending = engineerTasks.filter(t => t.status === 'pending').length;
      const engineerInProgress = engineerTasks.filter(t => t.status === 'in-progress').length;
      const engineerCompleted = engineerTasks.filter(t => t.status === 'completed').length;

      return {
        engineer,
        total: engineerTotal,
        pending: engineerPending,
        inProgress: engineerInProgress,
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-xl shadow-2xl max-w-5xl w-full max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-200 bg-gradient-to-r from-main to-main-700">
          <div>
            <h2 className="text-2xl font-bold text-white">Engineers Summary</h2>
        ...
        <div className="flex-1 overflow-y-auto p-6 bg-gray-50">
          <div className="space-y-3">
            {getEngineersSummaryPerEngineer().map(({ engineer, total, pending, inProgress, completed, months }) => (
              <div key={engineer} className="bg-white rounded-lg shadow-sm border border-gray-200 hover:shadow-md transition-shadow">
                {/* Engineer Header */}
                <div className="p-4">
                  <div className="flex items-center justify-between mb-3">
                    <div className="flex-1 min-w-0">
                      <div className="text-base font-semibold text-gray-900 truncate" title={engineer}>
                        {engineer}
                      </div>
                  ...
                  {months.length === 0 && (
                    <div className="pt-3 border-t border-gray-200">
                      <div className="text-sm text-gray-500 text-center py-2 italic">
                        No tasks found for this engineer in the selected year
                      </div>
                    </div>
                  )}
                </div>
